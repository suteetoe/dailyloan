...
namespace DailyLoan.Report
{
    public class report_ar_contract_payment : BizFlowControl.ReportRender
    {
        ...
        public const string REPORT_NAME = "รายงานการรับชำระค่างวด";
        ...
        public report_ar_contract_payment()
        {
            ...
            this.ReportColumns.Add(new BizFlowControl.ReportColumn() { HeaderText = "ยอดชำระ", ColumnWidth = 15, DataField = "amount" });
        ...
        protected override bool StartProcess()
        {
            try
            {
                ...
                string query =
                    ...
                    select id, doc_date, doc_time, doc_no, contract_no, customer_code, customer_name, total_amount
                ...
                if (ds.Tables.Count > 0)
                {
                    ...
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        row["customer"] = string.Format("{0} {1}", dr["customer_code"], dr["customer_name"]);
